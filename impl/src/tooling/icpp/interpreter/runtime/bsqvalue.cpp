...
{
    auto etype = BSQType::g_typetable[dynamic_cast<const BSQPartialVectorType*>(btype)->elemtype];
    auto count = *((uint32_t*)btype->indexStorageLocationOffset(data, 4));
    ...
    for(size_t i = 0; i < count; ++i)
    {
        ...
        auto mm = *((BSQBool*)btype->indexStorageLocationOffset(data, i));
        if(mm == BSQFALSE)
        {
            res += "_";
        }
        ...
        {
            auto idx = 8 + (i * etype->allocinfo.inlinedatasize);
            auto idata = btype->indexStorageLocationOffset(data, idx);
            res += etype->fpDisplay(etype, idata);
        }
    }
    res += "}";
...
std::string entityListPVHelperDisplay_impl(const BSQType* btype, StorageLocationPtr data)
{
    auto etype = BSQType::g_typetable[dynamic_cast<const BSQPartialVectorType*>(btype)->elemtype];
    auto count = *((uint32_t*)btype->indexStorageLocationOffset(data, 4));
...
std::string entityListTreeHelperDisplay_impl(const BSQType* btype, StorageLocationPtr data)
{
    auto ldata = btype->indexStorageLocationOffset(data, 8);
    auto ltype = SLPTR_LOAD_HEAP_TYPE(ldata);
    auto lstr = ltype->tid == btype->tid ? entityListTreeHelperDisplay_impl(ltype, ldata) : entityListPVHelperDisplay_impl(ltype, ldata);

    auto rdata = btype->indexStorageLocationOffset(data, 16);
    auto rtype = SLPTR_LOAD_HEAP_TYPE(rdata);
    auto rstr = rtype->tid == btype->tid ? entityListTreeHelperDisplay_impl(rtype, rdata) : entityListPVHelperDisplay_impl(rtype, rdata);
...
std::string entityListDisplay_impl(const BSQType* btype, StorageLocationPtr data)
{
    ...
    auto oftype = SLPTR_LOAD_UNION_INLINE_TYPE(data);
    if(oftype->tid == BSQ_TYPE_ID_NONE)
    {
        return btype->name + "{}";
    }
    else
    {
        if(oftype->tid == ltype->pvtype)
        {
            return btype->name + "{ " + entityListPVHelperDisplay_impl(BSQType::g_typetable[ltype->pvtype], SLPTR_LOAD_UNION_INLINE_DATAPTR(data)) + " }";
        }
        else
        {
            return btype->name + "{ " + entityListTreeHelperDisplay_impl(BSQType::g_typetable[ltype->treetype], SLPTR_LOAD_UNION_INLINE_DATAPTR(data)) + " }";
        }
    }
}


std::string entityMapTreeHelperDisplay_impl(const BSQType* btype, const BSQType* ktype, const BSQType* vtype, StorageLocationPtr data)
{
    auto ldata = btype->indexStorageLocationOffset(data, ktype->allocinfo.inlinedatasize + vtype->allocinfo.inlinedatasize);
    auto ltype = SLPTR_LOAD_UNION_INLINE_TYPE(ldata);
    auto lstr = ltype->tid != BSQ_TYPE_ID_NONE ? entityMapTreeHelperDisplay_impl(ltype, ktype, vtype, ldata) : "";

    auto rdata = btype->indexStorageLocationOffset(data, ktype->allocinfo.inlinedatasize + vtype->allocinfo.inlinedatasize + 16);
    auto rtype = SLPTR_LOAD_UNION_INLINE_TYPE(rdata);
    auto rstr = rtype->tid != BSQ_TYPE_ID_NONE ? entityMapTreeHelperDisplay_impl(rtype, ktype, vtype, rdata) : "";
    
    auto kvstr = ktype->fpDisplay(ktype, data) + " => " + vtype->fpDisplay(vtype, btype->indexStorageLocationOffset(data, ktype->allocinfo.inlinedatasize)); 

    return lstr + (!lstr.empty() ? ", " : "") + kvstr + (!rstr.empty() ? ", " : "") + rstr;
}
...
std::string entityMapDisplay_impl(const BSQType* btype, StorageLocationPtr data)
{
    ...
    else
    {
        ...
        auto vtype = BSQType::g_typetable[mtype->vtype];
        return btype->name + "{ " + entityMapTreeHelperDisplay_impl(BSQType::g_typetable[mtype->treetype], ktype, mtype, SLPTR_LOAD_UNION_INLINE_DATAPTR(data)) + " }";
    }
}

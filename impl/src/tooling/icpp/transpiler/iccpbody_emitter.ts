...
class ICPPBodyEmitter {
    ...
    generateUpdateEntityFieldDirect(geninfo: { inv: string, arglayouttype: MIRType, argflowtype: MIRType, updates: [MIRFieldKey, MIRResolvedTypeKey][], resulttype: MIRType }, sinfo: SourceInfo): ICPPInvokeDecl {
        ...
        edecl.consfuncfields.forEach((cff) => {
            ...
            if (updidx !== -1) {
                pargs.push(this.getStackInfoForArgVar(`arg_${f}`))
            }
            else {
    ...
    processEntityUpdate(op: MIREntityUpdate): ICPPOp {
        ...
        if (op.isvirtual) {
            ...
            if(!entity.hasconsinvariants) {
                ...
                return ICPPOpEmitter.genUpdateEntityOp(op.sinfo, this.trgtToICPPTargetLocation(op.trgt, op.argflowtype), resulttype.typeID, this.argToICPPLocation(op.arg), op.arglayouttype, op.argflowtype, updates);
            }
            else {
                ...
                return ICPPOpEmitter.genInvokeFixedFunctionOp(op.sinfo, this.trgtToICPPTargetLocation(op.trgt, op.argflowtype), resulttype.typeID, icall, [this.argToICPPLocation(op.arg)], -1, ICPPOpEmitter.genNoStatmentGuard());
            }
        }
    }
